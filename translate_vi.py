...
# ///
"""
Translate CS50 English content to Vietnamese using Gemini CLI.
Strategy:
  - Batch multiple small files (<3000 chars) into one Gemini call
  - For large files (notes), translate prose sections, keep code blocks unchanged
  - Skip files that already exist in vi/ with --skip-existing
"""
...
SRC = Path(__file__).parent / "content/en"
DST = Path(__file__).parent / "content/vi"

BATCH_PROMPT = """Translate the following CS50 course Markdown files from English to Vietnamese.
...
6. Write fluent, natural Vietnamese — as a skilled Vietnamese CS educator would write
7. Output ONLY the translated files separated by ---FILE-SEPARATOR--- markers, no extra commentary

"""

LARGE_FILE_PROMPT = """Translate this CS50 course Markdown file from English to Vietnamese.
...
def run_gemini(prompt: str, stdin_text: str = "") -> str:
    full_input = stdin_text if stdin_text else ""
    ...
    # Strip MCP warning line if present
...
def translate_large_file(raw: str) -> str:
    """Translate a large file in a single Gemini call."""
    return run_gemini(LARGE_FILE_PROMPT + raw)


def batch_translate(batch: list[tuple[Path, Path]]) -> None:
    """Translate a batch of small files in a single Gemini call."""
    parts = []
    for src, _ in batch:
...
def main():
    ...
    print(f"Files: {len(small_q)} small (→{batch_calls} batched calls), {len(large_q)} large (sectioned)")
    print(f"Total Gemini calls estimate: ~{batch_calls + len(large_q) * 4}\n")
